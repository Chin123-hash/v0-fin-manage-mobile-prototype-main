...
import { userProfile, savingStats, rewards, formatRM } from "@/lib/mock-data";
import { 
  Settings, 
  Users, 
  Calendar, 
    ...
    <SafeAreaView className="flex-1 bg-background" edges={["top"]}>
      ...
      <ScrollView className="flex-1 px-6" showsVerticalScrollIndicator={false}>
        ...
        <View className="mb-8">
          ...
          {/* Interactive Tank Navigation */}
          <TouchableOpacity 
            onPress={() => router.push('/pet-hub')}
            activeOpacity={0.9}
            className="shadow-lg shadow-black/40"
          >
            <DigitalTank height={160} showFullTank={showGroupTank} />
          </TouchableOpacity>
        </View>

        <View className="flex-row flex-wrap justify-between mb-8">
          <StatCard icon={<Zap size={18} color={colors.accent.teal} />} label="Saving Streak" value={`${savingStats.streak} Days`} />
          <StatCard icon={<Calendar size={18} color={colors.accent.teal} />} label="Days Joined" value="128 Days" />
          <StatCard icon={<Trophy size={18} color={colors.accent.gold} />} label="Fin Managed" value={formatRM(userProfile.totalBalance)} />
          <StatCard icon={<Users size={18} color={colors.accent.pink} />} label="Friends" value="12" />
        </View>

        <View className="mb-10">
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-foreground font-bold text-lg">Achievements</Text>
            <TouchableOpacity><Text className="text-accent text-sm font-bold">View All</Text></TouchableOpacity>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} className="-mx-6 px-6">
            {rewards.filter(r => r.isUnlocked).map((reward) => (
              <View key={reward.id} className="items-center mr-6">
                ...
                <Text className="text-foreground text-[10px] font-bold text-center w-16" numberOfLines={1}>{reward.name}</Text>
              </View>
            ))}
          </ScrollView>
        </View>
